...
class CompareFile:
    '''
    first compare size, then compared hashed value when size is same
    '''
    ...
    def lazy_init(self, progress_hint: pyqtSignal, progress_int: pyqtSignal):
        ...
        self.file_list = self.file_obj.contents  # init complete
    ...
    def _compare_hash(self, size_res: dict):
        ...
        for idx, (size, file_list) in enumerate(size_res.items()):
            ...
            # classify files with same hash value
            ...
            # if the same size captured more than one, add to list
