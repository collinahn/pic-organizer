...
class ViewListDialog(QDialog):
    ...
    def _init_ui(self):
        ...
        lbl_explain = QLabel(
            '파일명을 클릭하면 사진을 확인할 수 있습니다. 미리보기를 더블 클릭하면 해당 폴더로 이동합니다.')
        ...
        for idx_group, overlapped in enumerate(self.data):
            ...
            if self.meta_data[idx_group]:
                chkbox_flag.setChecked(True)
            chkbox_flag.toggled.connect(self.on_chkbox_toggle)
            layout_overlapped.addWidget(
                chkbox_flag, 0, 0, 100, 1, alignment=Qt.AlignmentFlag.AlignVCenter)
            ...
            for idx_fname, file in enumerate(overlapped):
                ...
                layout_overlapped.addWidget(
                    btn_filename, idx_fname, 1, 1, 100, alignment=Qt.AlignmentFlag.AlignLeft)
        ...
        self.scroll_files_view.setVerticalScrollBarPolicy(
            Qt.ScrollBarPolicy.ScrollBarAlwaysOn)
        self.scroll_files_view.setHorizontalScrollBarPolicy(
            Qt.ScrollBarPolicy.ScrollBarAlwaysOn)
        ...
        self.pixmap_preview.setFixedSize(
            self.scroll_files_view.width(), self.scroll_files_view.height())
        ...
        dialog_layout.addWidget(
            btn_exit, 2, 1, alignment=Qt.AlignmentFlag.AlignRight)
    ...
    def on_btn_filename(self):
        ...
        ref_size = (self.scroll_files_view.width(),
                    self.scroll_files_view.height())
        self.pixmap_preview.setMaximumSize(*ref_size)
        self.pixmap_preview.setPixmap(QPixmap(self.current_file).scaled(
            *ref_size, Qt.AspectRatioMode.KeepAspectRatio))
        if self.pixmap_preview.pixmap().isNull():
            self.pixmap_preview.setText(
                '지원하지 않는 파일 형식입니다.\n이 영역을 더블 클릭하면 해당 폴더로 이동합니다.')

    def on_doubleclick_pixmap(self, event: QMouseEvent):
        if platform.system() == "Darwin":
            if Browser.korean_included_for_mac(self.current_file):
                DefaultDialog(
                    '현재 맥에서는 한글이 포함된 경로 폴더 보기는 지원되지 않습니다.', ('네', )).exec()
                return
...
if __name__ == '__main__':
    ...
    ViewListDialog([('123', '1234'), ('123456', '12343535',
                   '123424123424123424123424123424123424123424123424123424123424123424123424123424123424123424123424123424123424', '12313213'), ], [True, True]).exec()
    sys.exit(app.exec())
